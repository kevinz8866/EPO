...
class EPOTrainer(Trainer):
    r"""
    EPO trainer modified based on DPOTrainer.
    ...
    def dpo_loss(
        ...
        policy_alignment_loss: torch.FloatTensor,
        ...
        if self.loss_type == "sigmoid":
            losses = -F.logsigmoid(self.beta * logits) +  torch.exp(policy_chosen_logps)
    ...
    def _get_alignment_loss(
        ...
        alignment_loss = F.cross_entropy(flat_logits, flat_labels)
    ...
    def concatenated_forward(
        ...
        if torch.any(torch.isnan(all_logits)):
            print()
        ...
        alignment_loss = self._get_alignment_loss(
            all_logits[:len_chosen],
            concatenated_batch["concatenated_labels"][:len_chosen]
        )
        
        if torch.any(torch.isnan(all_logps)):
            print('logp has nan')
        ...
        return (chosen_logps, rejected_logps, chosen_logits, rejected_logits, alignment_loss)
